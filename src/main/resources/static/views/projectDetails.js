    ...
    return `
        <div class="container-fluid">
            ...
            <div class="row m-3">
                ...
                ${project.issues.length > 0 ? `
                    <ul class="list-group scrollarea">
                        ${project.issues.map(issue => `
                            <li class="list-group-item d-flex justify-content-between align-items-center list-group-item-action" href="/issues/${issue.id}" data-link aria-current="true">
                                <h5>${getStatusIcon(issue.status)} <strong>${issue.title}:</strong> ${issue.description}</h5>
                                <span class="badge text-bg-primary rounded-pill">${issue.comments.length}</span>
                            </li>
                        `).join("")}
                    </ul>
                ...
                <div class="col-4" style="width: 380px;">
